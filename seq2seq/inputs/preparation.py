    ...
    * Relation File: this file records the relationship between two texts, each line containing the label and a pair of ids.
    ...
    def get_text_id(self, hashid, text, idtag='T'):
        hash_obj = hashlib.sha1(text.encode(
            'utf8'))  # if the text are the same, then the hash_code are also the same
    ...
    def parse_line(self, line, delimiter='\t'):
        ...
        if 3 != len(subs):
            raise ValueError(
                'format of data file wrong, should be \'label,text1,text2\'.')
        else:
    ...
    def run_with_train_valid_test_corpus(self, train_file, valid_file,
                                         test_file):
        '''
    ...
    def merge_corpus(train_corpus, valid_corpus, test_corpus):
        ...
        os.system(
            'cat ' + train_corpus + ' ' + valid_corpus + ' ' + test_corpus + '  > corpus.txt')
            ...
            for l in f_in:
                ...
                if tokens[1] == cur_qid:
                    ...
                    if not found_dup_doc:
                        ...
                        print('found qid with duplicated doc id/text: ',
                              ''.join(cache_q_lines))
