...
class PointGenerator(object):
    def __init__(self, config):
        ...
        self.check_list = ['data1', 'data2', 'text1_maxlen', 'text2_maxlen',
                           'relation_file', 'batch_size', 'vocab_size']
        if not self.check():
...
class Triletter_PointGenerator(object):
    def __init__(self, config):
        ...
        self.check_list = ['data1', 'data2', 'text1_maxlen', 'text2_maxlen',
                           'relation_file', 'batch_size', 'vocab_size',
                           'dtype', 'word_triletter_map_file']
        if not self.check():
            raise TypeError(
                '[Triletter_PointGenerator] parameter check wrong.')
        ...
        if self.dtype == 'dssm':
            ...
            return self.transfer_feat2fixed(X1, self.data1_maxlen,
                   ...
                   self.transfer_feat2fixed(X2, self.data2_maxlen,
                                            self.fill_word), X2_len, Y, ID_pairs
...
class DRMM_PointGenerator(object):
    ...
    def get_batch(self, randomly=True):
        ...
        X1 = np.zeros((self.batch_size, self.data1_maxlen), dtype=np.int32)
        X1_len = np.zeros((self.batch_size,), dtype=np.int32)
        X2 = np.zeros((self.batch_size, self.data1_maxlen, self.hist_size),
                      dtype=np.float32)
        X2_len = np.zeros((self.batch_size,), dtype=np.int32)
        if self.target_mode == 'regression':
            Y = np.zeros((self.batch_size,), dtype=np.int32)
        ...
        for i in range(self.batch_size):
            ...
            d2_len = min(self.data2_maxlen, len(self.data2[d2]))
            X1[i, :d1_len], X1_len[i] = self.data1[d1][:d1_len], d1_len
            X2[i], X2_len[i] = self.cal_hist(d1, d2, self.data1_maxlen,
