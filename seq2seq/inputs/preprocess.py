...
class Preprocess(object):
    ...
    def word_seg(docs, config):
        assert config[
                   'lang'].lower() in Preprocess._valid_lang, 'Wrong language type: %s' % \
                                                              config['lang']
        docs = getattr(Preprocess, '%s_%s' % (
            sys._getframe().f_code.co_name, config['lang']))(docs)
        return docs
    ...
    def word_filter(docs, config, words_stats):
        if config['words_useless'] is None:
            ...
            for w, winfo in words_stats.items():
                ...
                if config['min_freq'] > winfo['df'] or config['max_freq'] < \
                        winfo['df']:
                    config['words_useless'].add(w)
        # filter with useless words
        docs = [[w for w in ws if w not in config['words_useless']] for ws in
                tqdm(docs)]
    ...
    def save_words_stats(self, words_stats_fp, sort=False):
        if sort:
            word_dic = sorted(self._word_dict.items(), key=lambda d: d[1],
                              reverse=False)
            lines = ['%s %d %d %f' % (
            ...
            lines = ['%s %d %d %f' % (
                wid, self._words_stats[w]['cf'], self._words_stats[w]['df'],
                self._words_stats[w]['idf']) for w, wid in
                     self._word_dict.items()]
...
class NgramUtil(object):
            ...
            for i in xrange(L - 2):
                for j in xrange(i + 1, L - 1):
                    for k in xrange(j + 1, L):
                        lst.append(
                            join_string.join([words[i], words[j], words[k]]))
        else:
            # set it as biterm
    ...
    def ngrams(words, ngram, join_string=" "):
        ...
        if ngram == 1:
            ...
            bigram = [x for x in NgramUtil.bigrams(words, join_string) if
                      len(x.split(join_string)) == 2]
            return unigram + bigram
        elif ngram == 123:
            unigram = NgramUtil.unigrams(words)
            bigram = [x for x in NgramUtil.bigrams(words, join_string) if
                      len(x.split(join_string)) == 2]
            trigram = [x for x in NgramUtil.trigrams(words, join_string) if
                       len(x.split(join_string)) == 3]
