    ...
    def get_batch(self):
        while self.point < self.num_list:
            ...
            if self.point + self.batch_list <= self.num_list:
                currbatch = self.list_list[
                            self.point: self.point + self.batch_list]
            ...
            if self.config['use_dpool']:
                yield ({
                           ...
                           'dpool_index': DynamicMaxPooling.dynamic_pooling_index(
                               ...
                               self.config['text2_maxlen']), 'ID': ID_pairs,
                           'list_counts': list_counts
                       }, Y)
            else:
                yield ({
                           'query': X1, 'query_len': X1_len, 'doc': X2,
                           'doc_len': X2_len, 'ID': ID_pairs,
                           'list_counts': list_counts
                       }, Y)
        ...
        while self.point < self.num_list:
            ...
            if self.dtype == 'dssm':
                yield self.transfer_feat2sparse(
                    ...
                    X2).toarray(), X2_len, Y, ID_pairs, list_count
    ...
    def get_all_data(self):
        ...
        while self.point < self.num_list:
            ...
            if self.type == 'dssm':
                ...
                x1_ls.append(self.transfer_feat2fixed(X1, self.data1_maxlen,
                                                      self.fill_word))
                x2_ls.append(self.transfer_feat2fixed(X2, self.data2_maxlen,
                                                      self.fill_word))
            x1_len_ls.append(X1_len)
            x2_len_ls.append(X2_len)
            y_ls.append(Y)
...
class DRMM_ListGenerator(ListBasicGenerator):
    def __init__(self, config={}):
        ...
        print('[DRMM_ListGenerator] init done, list number: %d. ' % (
            self.num_list), end='\n')
    ...
    def cal_hist(self, t1, t2, data1_maxlen, hist_size):
        ...
        if self.use_hist_feats:
            ...
            caled_hist = np.reshape(self.hist_feats[(t1, t2)],
                                    (d1len, hist_size))
    ...
    def get_batch(self):
        while self.point < self.num_list:
            ...
            X1 = np.zeros((bsize, self.data1_maxlen), dtype=np.int32)
            X1_len = np.zeros((bsize,), dtype=np.int32)
            X2 = np.zeros((bsize, self.data1_maxlen, self.hist_size),
                          dtype=np.float32)
            X2_len = np.zeros((bsize,), dtype=np.int32)
            Y = np.zeros((bsize,), dtype=np.int32)
            X1[:] = self.fill_word
            ...
            for pt in currbatch:
                ...
                for l, d2 in d2_list:
                    ...
                    d2_cont = list(self.data2[d2])
                    d2_len = len(d2_cont)
                    X2[j], X2_len[j] = self.cal_hist(d1, d2, self.data1_maxlen,
                                                     self.hist_size), d2_len
                    ID_pairs.append((d1, d2))
                    Y[j] = l
                    j += 1
            yield X1, X1_len, X2, X2_len, Y, ID_pairs, list_count
...
class ListGenerator_Feats(ListBasicGenerator):
        ...
        while self.point < self.num_list:
            ...
            for pt in currbatch:
                ...
                for l, d2 in d2_list:
                    ...
                    X4[j, :d1_len] = self.query_feats[d1][
    ...
    def get_batch_generator(self):
        for X1, X1_len, X2, X2_len, X3, X4, Y, ID_pairs, list_counts in self.get_batch():
            yield ({
                       'query': X1, 'query_len': X1_len, 'doc': X2,
                       'doc_len': X2_len, 'pair_feats': X3, 'query_feats': X4,
                       'ID': ID_pairs, 'list_counts': list_counts
                   }, Y)
