...
class PairBasicGenerator(object):
    ...
    def get_all_pairs(self):
        return self.make_pair_static(self.rel)
    ...
    def make_pair_iter(self, rel):
        ...
        while True:
            rel_set_sample = random.sample(rel_set.keys(),
                                           self.config['query_per_iter'])
...
class PairGenerator(PairBasicGenerator):
    def __init__(self, config):
        ...
        self.check_list.extend(
            ['data1', 'data2', 'text1_maxlen', 'text2_maxlen'])
        if config['use_iter']:
    ...
    def get_batch_static(self):
        ...
        for i in range(self.batch_size):
            ...
            X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = d1_cont[
                                                        :d1_len], d1_len
            X2[i * 2 + 1, :d2n_len], X2_len[i * 2 + 1] = d2n_cont[
                                                         :d2n_len], d2n_len
    ...
    def get_batch_iter(self):
        while True:
            ...
            for _ in range(self.config['batch_per_iter']):
                X1 = np.zeros((self.batch_size * 2, self.data1_maxlen),
                              dtype=np.int32)
                X1_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
                X2 = np.zeros((self.batch_size * 2, self.data2_maxlen),
                              dtype=np.int32)
                X2_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
                Y = np.zeros((self.batch_size * 2,), dtype=np.int32)
                ...
                for i in range(self.batch_size):
                    ...
                    d2p_len = min(self.data2_maxlen,
                                  len(list(self.data2[d2p])))
                    d2n_len = min(self.data2_maxlen,
                                  len(list(self.data2[d2n])))
                    X1[i * 2, :d1_len], X1_len[i * 2] = self.data1[d1][
                                                        :d1_len], d1_len
                    X2[i * 2, :d2p_len], X2_len[i * 2] = self.data2[d2p][
                                                         :d2p_len], d2p_len
                    X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = self.data1[d1][
                                                                :d1_len], d1_len
                    X2[i * 2 + 1, :d2n_len], X2_len[i * 2 + 1] = self.data2[
                                                                     d2n][
                                                                 :d2n_len], d2n_len
    ...
    def get_batch_generator(self):
        while True:
            ...
            if self.config['use_dpool']:
                yield ({
                           'query': X1, 'query_len': X1_len, 'doc': X2,
                           'doc_len': X2_len,
                           'dpool_index': DynamicMaxPooling.dynamic_pooling_index(
...
class Triletter_PairGenerator(PairBasicGenerator):
    def __init__(self, config):
        ...
        self.check_list.extend(['data1', 'data2', 'dtype', 'vocab_size',
                                'word_triletter_map_file'])
        if config['use_iter']:
            self.batch_iter = self.get_batch_iter()
        if not self.check():
            raise TypeError('[Triletter_PairGenerator] parameter check wrong.')
        self.word_triletter_map = self.read_word_triletter_map(
            self.config['word_triletter_map_file'])
    ...
    def transfer_feat2sparse(self, dense_feat):
        ...
        res = sp.csr_matrix((data, indices, indptr),
                            shape=(len(dense_feat), self.vocab_size),
                            dtype="float32")
    ...
    def get_batch_static(self):
        ...
        if self.dtype == 'dssm':
            return self.transfer_feat2sparse(
                X1).toarray(), X1_len, self.transfer_feat2sparse(
                X2).toarray(), X2_len, Y
        elif self.dtype == 'cdssm':
            return self.transfer_feat2fixed(X1, self.data1_maxlen,
                                            self.fill_word), X1_len, \
                   self.transfer_feat2fixed(X2, self.data2_maxlen,
                                            self.fill_word), X2_len, Y
    ...
    def get_batch_iter(self):
        while True:
            ...
            for _ in range(self.config['batch_per_iter']):
                ...
                if self.dtype == 'dssm':
                    yield self.transfer_feat2sparse(
                        X1).toarray(), X1_len, self.transfer_feat2sparse(
                        X2).toarray(), X2_len, Y
                elif self.dtype == 'cdssm':
                    yield self.transfer_feat2fixed(X1, self.data1_maxlen,
                                                   self.fill_word), X1_len, \
                          self.transfer_feat2fixed(X2, self.data2_maxlen,
                                                   self.fill_word), X2_len, Y
...
class DRMM_PairGenerator(PairBasicGenerator):
    def __init__(self, config):
        ...
        self.fill_word = config['vocab_size'] - 1
        self.check_list.extend(
            ['data1', 'data2', 'text1_maxlen', 'text2_maxlen', 'embed'])
    ...
    def get_batch_static(self):
        X1 = np.zeros((self.batch_size * 2, self.data1_maxlen), dtype=np.int32)
        X1_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
        X2 = np.zeros((self.batch_size * 2, self.data1_maxlen, self.hist_size),
                      dtype=np.float32)
        X2_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
        Y = np.zeros((self.batch_size * 2,), dtype=np.int32)
        ...
        for i in range(self.batch_size):
            ...
            X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = d1_cont[
                                                        :d1_len], d1_len
            X2[i * 2], X2_len[i * 2] = self.cal_hist(d1, d2p,
                                                     self.data1_maxlen,
                                                     self.hist_size), d2p_len
            X2[i * 2 + 1], X2_len[i * 2 + 1] = self.cal_hist(d1, d2n,
                                                             self.data1_maxlen,
                                                             self.hist_size), d2n_len
    ...
    def get_batch_iter(self):
        while True:
            ...
            for _ in range(self.config['batch_per_iter']):
                ...
                X1_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
                X2 = np.zeros(
                    (self.batch_size * 2, self.data1_maxlen, self.hist_size),
                    dtype=np.float32)
                X2_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
                Y = np.zeros((self.batch_size * 2,), dtype=np.int32)
                ...
                for i in range(self.batch_size):
                    ...
                    d2p_len = len(d2p_cont)
                    d2n_len = len(d2n_cont)
                    X1[i * 2, :d1_len], X1_len[i * 2] = d1_cont[
                                                        :d1_len], d1_len
                    X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = d1_cont[
                                                                :d1_len], d1_len
                    X2[i * 2], X2_len[i * 2] = self.cal_hist(d1, d2p,
...
class PairGenerator_Feats(PairBasicGenerator):
    def __init__(self, config):
        ...
        self.check_list.extend(
            ['data1', 'data2', 'text1_maxlen', 'text2_maxlen',
             'pair_feat_size', 'pair_feat_file', 'query_feat_size',
             'query_feat_file'])
        if not self.check():
            raise TypeError('[PairGenerator] parameter check wrong.')
    ...
    def get_batch_static(self):
        ...
        X2 = np.zeros((self.batch_size * 2, self.data2_maxlen), dtype=np.int32)
        X2_len = np.zeros((self.batch_size * 2,), dtype=np.int32)
        X3 = np.zeros((self.batch_size * 2, self.pair_feat_size),
                      dtype=np.float32)
        X4 = np.zeros((self.batch_size * 2, self.query_feat_size),
                      dtype=np.float32)
        Y = np.zeros((self.batch_size * 2,), dtype=np.int32)
        ...
        for i in range(self.batch_size):
            ...
            X2[i * 2, :d2p_len], X2_len[i * 2] = self.data2[d2p][
                                                 :d2p_len], d2p_len
            X3[i * 2, :self.pair_feat_size] = self.pair_feats[(d1, d2p)][
                                              :self.pair_feat_size]
            X4[i * 2, :self.query_feat_size] = self.query_feats[d1][
                                               :self.query_feat_size]
            X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = self.data1[d1][
                                                        :d1_len], d1_len
            X2[i * 2 + 1, :d2n_len], X2_len[i * 2 + 1] = self.data2[d2n][
                                                         :d2n_len], d2n_len
            X3[i * 2 + 1, :self.pair_feat_size] = self.pair_feats[(d1, d2n)][
                                                  :self.pair_feat_size]
            X4[i * 2 + 1, :self.query_feat_size] = self.query_feats[d1][
                                                   :self.query_feat_size]
        ...
        while True:
            ...
            for _ in range(self.config['batch_per_iter']):
                ...
                for i in range(self.batch_size):
                    ...
                    X2[i * 2, :d2p_len], X2_len[i * 2] = self.data2[d2p][
                                                         :d2p_len], d2p_len
                    X3[i * 2, :self.pair_feat_size] = self.pair_feats[
                                                          (d1, d2p)][
                                                      :self.pair_feat_size]
                    X4[i * 2, :d1_len] = self.query_feats[d1][
                                         :self.query_feat_size]
                    X1[i * 2 + 1, :d1_len], X1_len[i * 2 + 1] = self.data1[d1][
                                                                :d1_len], d1_len
                    X2[i * 2 + 1, :d2n_len], X2_len[i * 2 + 1] = self.data2[
                                                                     d2n][
                                                                 :d2n_len], d2n_len
                    X3[i * 2 + 1, :self.pair_feat_size] = self.pair_feats[
                                                              (d1, d2n)][
                                                          :self.pair_feat_size]
                    X4[i * 2 + 1, :d1_len] = self.query_feats[d1][
                                             :self.query_feat_size]
    ...
    def get_batch_generator(self):
        while True:
            ...
            yield ({
                       'query': X1, 'query_len': X1_len, 'doc': X2,
                       'doc_len': X2_len, 'query_feats': X4, 'pair_feats': X3
                   }, Y)
